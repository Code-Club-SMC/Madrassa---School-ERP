...
import { Search, Receipt, Printer, Wallet, CheckCircle2 } from "lucide-react";
...
import { feeRecords, students, institution, schoolClasses, type FeeRecord } from "@/mock";
...
const schoolRecords = feeRecords.filter((r) => {
  const s = students.find((x) => x.id === r.studentId);
  return s?.system === "school";
});
...
function SchoolFeesPage() {
  const [records, setRecords] = useState(schoolRecords);
  ...
  const totals = useMemo(() => ({
    ...
    outstanding: records.reduce((a, r) => a + (r.monthlyFee - r.paidAmount), 0),
  }), [records]);
  ...
  const classNameOf = (id: string) => schoolClasses.find((c) => c.id === id)?.nameUrdu ?? id;
  ...
  const collect = (r: FeeRecord) => {
    ...
    setReceipt({ ...r, paidAmount: r.monthlyFee, status: "paid", paidOn: new Date().toISOString() });
    toast.success(`Collected ${formatPKR(r.monthlyFee)}`, { description: "اسکول کی فیس وصول ہوگئی" });
  };

  return (
    <div>
      <PageHeader title="School Fees" titleUrdu="اسکول — فیس" description="Class-wise monthly fees with receipts and defaulter watch." />

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
        <Card className="p-4"><p className="text-xs text-muted-foreground">Total Due · کل واجب</p><p className="font-heading text-2xl font-bold mt-1">{formatPKR(totals.expected)}</p></Card>
        ...
        <Card className="p-4"><p className="text-xs text-muted-foreground">Outstanding · بقایا</p><p className="font-heading text-2xl font-bold mt-1 text-destructive">{formatPKR(totals.outstanding)}</p></Card>
      </div>
      ...
      <Card className="overflow-hidden">
        <Table>
          ...
          <TableBody>
            {filtered.length === 0 ? (
              <TableRow><TableCell colSpan={7} className="py-12"><EmptyState icon={Wallet} heading="No fee records" headingUrdu="کوئی ریکارڈ نہیں" /></TableCell></TableRow>
            ) : filtered.map((r) => (
              <TableRow key={r.id}>
                <TableCell className="font-mono text-xs">{r.rollNo}</TableCell>
                <TableCell><p className="font-urdu text-sm">{r.nameUrdu}</p></TableCell>
                <TableCell><p className="font-urdu text-sm">{classNameOf(r.subgroup)}</p></TableCell>
      ...
      <Dialog open={!!receipt} onOpenChange={(v) => !v && setReceipt(null)}>
        <DialogContent className="max-w-md print:shadow-none">
          ...
          {receipt && (
            <div className="print-target rounded-xl border border-border p-5 space-y-3">
              <div className="text-center pb-3 border-b border-border">
                <p className="font-urdu text-lg font-bold">{institution.nameUrdu}</p>
                <p className="text-xs uppercase tracking-widest text-muted-foreground">{institution.nameEnglish} · School</p>
              </div>
              <Row label="Receipt No." urdu="رسید نمبر" value={`R-${receipt.id.toUpperCase()}`} mono />
              <Row label="Date" urdu="تاریخ" value={formatDate(receipt.paidOn ?? new Date())} />
              <Row label="Student" urdu="طالب علم" value={receipt.nameUrdu} urduValue />
              <Row label="Class" urdu="جماعت" value={classNameOf(receipt.subgroup)} urduValue />
              <Row label="Roll" urdu="رول" value={receipt.rollNo} mono />
              <Row label="Month" urdu="ماہ" value={receipt.month} mono />
              <div className="pt-3 mt-3 border-t border-border flex items-center justify-between">
                <div><p className="text-xs text-muted-foreground">Amount Paid · ادا کردہ</p><p className="font-urdu text-sm">شکریہ</p></div>
...
function Row({ label, urdu, value, mono, urduValue }: { label: string; urdu: string; value: string; mono?: boolean; urduValue?: boolean }) {
  return (
